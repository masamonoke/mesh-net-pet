...
void format_app_create_message(const struct app_payload* app_payload, uint8_t* p) {
	...
	memcpy(p, &app_payload->label_from, sizeof(app_payload->label_from));
	p += sizeof(app_payload->label_from);
	memcpy(p, &app_payload->label_to, sizeof(app_payload->label_to));
	p += sizeof(app_payload->label_to);
...
void format_app_parse_message(void* payload, const uint8_t* p) {
	...
	p += sizeof_enum(app_payload->req_type);
	memcpy(&app_payload->label_from, p, sizeof(app_payload->label_from));
	p += sizeof(app_payload->label_from);
	memcpy(&app_payload->label_to, p, sizeof(app_payload->label_to));
	p += sizeof(app_payload->label_to);
...
uint32_t format_app_message_len(struct app_payload* payload) {
	return sizeof(payload->label_from) + sizeof(payload->label_to) + sizeof(payload->key) +
