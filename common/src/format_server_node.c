...
static int32_t parse_message(enum request_type_server_node* request, void** payload, const uint8_t* buf, size_t buf_len);

static int32_t create_message(enum request_type_server_node request, const void* payload, uint8_t* message, uint32_t* msg_len);
...
static int32_t parse_message(enum request_type_server_node* request, void** payload, const uint8_t* buf, size_t buf_len) {
	...
	*request = REQUEST_TYPE_SERVER_NODE_UNDEFINED;
	p = buf;
	...
	if (buf_len == message_len) {
		...
		if (buf_len > cmd_len) {
			...
			if (0 == strcmp(cmd, "update_node") && 0 == strcmp(sender_str, "server")) {
				*request = REQUEST_TYPE_SERVER_NODE_UPDATE;
			} else if (0 == strcmp(cmd, "ping") && 0 == strcmp(sender_str, "server")) {
				*request = REQUEST_TYPE_SERVER_NODE_PING;
			} else {
				custom_log_debug("Unknown request format: cmd = %s", cmd);
			}
		} else {
	...
	switch (*request) {
		...
		case REQUEST_TYPE_SERVER_NODE_UNDEFINED:
			custom_log_error("Unknown server-node request type");
	}
		...
		case REQUEST_TYPE_SERVER_NODE_PING:
			{
				...
				*msg_len = cmd_len + sender_len + sizeof(cmd_len) + sizeof(*msg_len) + sizeof(sender_len);
